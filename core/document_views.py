...
class IsAdminManagerHRNoDelete(permissions.BasePermission):
    """Admin/manager/HR can manage documents; HR cannot delete."""

    def has_permission(self, request, view):
        user = request.user
        if not user or not user.is_authenticated:
            return False
        if request.method == 'DELETE' and getattr(user, 'role', None) == 'hr':
            return False
        return getattr(user, 'role', None) in ['admin', 'manager', 'hr']
...
class DocumentTemplateViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAdminManagerHRNoDelete]
    ...
    def get_queryset(self):
        ...
        if self.request.user.role in ['admin', 'manager', 'hr']:
...
class GeneratedDocumentViewSet(viewsets.ModelViewSet):
    ...
    search_fields = [
        'title', 'document_type', 'template__name',
        'employee__first_name', 'employee__last_name', 'employee__username',
        'employee__email', 'employee__employee_id', 'employee__biometric_id',
    ...
    def get_permissions(self):
        if self.action == 'destroy':
            return [IsAdminManagerHRNoDelete()]
    ...
    def generate_document(self, request):
        ...
        try:
            ...
            try:
                ...
                admins = CustomUser.objects.filter(role__in=['admin', 'hr'], is_active=True)
                for admin in admins:
